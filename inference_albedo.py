...
class MidasNet_small(BaseModel):
    def __init__(
        ...
        self.scratch.refinenet1 = FeatureFusionBlock_custom(features1, self.scratch.activation, deconv=False, bn=False, expand=self.expand, align_corners=align_corners)
...
def infer_hr_alb_batch(models, batch_tensor, resize_conf=None, base_size=384, linear=False):
    device = batch_tensor.device
    outputs = []
    with torch.no_grad():
        for item in batch_tensor:
            img = item.detach().permute(1, 2, 0).cpu().numpy()
            pred = run_pipeline(models, img, resize_conf=resize_conf, base_size=base_size, linear=linear, device=device)["hr_alb"]
...
def main():
    ...
    parser.add_argument("--input", default="009_009_rgb.png", help="Image file or folder of images.")
