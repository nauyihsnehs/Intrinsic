...
def save_outputs(hr_alb, output_path, output_image=None):
    ...
    if output_image:
        img = hr_alb ** (1 / 2.2)
        img = ((img / img.max()).clip(0.0, 1.0) * 255).astype(np.uint8)
...
def main():
    ...
    parser.add_argument("--image", default='009_009_rgb.png')
    ...
    parser.add_argument("--output", default="009_009_rgb.npy", help="Output .npy file for hr_alb.")
    parser.add_argument("--output-image", default="009_009_rgb-alb.png", help="Optional output PNG for hr_alb visualization.")
